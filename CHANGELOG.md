...
- A PAM grantee could not load queues, counter groups, the calendar, uploads, or open documents for collaborative editing — these returned 500s (or an "access denied" on the document WebSocket). Thirteen guild-scoped tables carried a legacy row-security policy that cast the active-guild session variable to an integer without guarding the empty value a grantee leaves it at, which faulted the entire query; the collaboration WebSocket additionally required real guild membership. Both are now grant-aware.
- A PAM grantee got 403s on an initiative's "my permissions" and member-roster endpoints (used to gate create buttons and populate assignee pickers/avatars), and a 500 when the app recorded a recent-view for a project/document/queue/counter they opened. Grantees now receive read-only initiative permissions (read-write grants can create), can load the member roster, and their transient browsing simply isn't recorded in recent views.
- A read-write PAM grant couldn't actually edit anything in the granted guild: the database and API already allowed the writes, but the per-item permission level the UI uses to show edit controls was computed only from a grantee's (nonexistent) membership permissions, so everything rendered read-only. Projects, documents, queues, and counter groups now report a read-write grant as `write` access (a read grant as `read`; a grant never reports `owner`), so edit affordances appear and match what the backend permits.
