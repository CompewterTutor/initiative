...
- **Guild context is now server-held — the `X-Guild-ID` header is gone.** The backend records which guild you're in (`users.active_guild_id`, `NULL` = personal/cross-guild mode), set by a new `PUT /users/me/guild-context` endpoint when you click a guild or the personal home page. Every surface — REST, websockets, document downloads, iframe/media loads, deep links — now resolves its guild from that one server-held flag instead of per-request headers, which removes a whole class of "this surface can't send the header" bugs (and the dangerous fallback that silently bound headerless requests to an arbitrary guild). Membership is still re-validated on every request, so a stale context fails closed. Cross-guild actions from personal pages (My Tasks status changes, the create wizards, closing a header tab) use explicit, validated `?guild_id=` addressing on a small named set of endpoints. The recent-tabs bar is now cross-guild by design: one user-scoped query returns tab names from all your guilds in any context, and opening a tab enters its guild before any content loads. Tabs in other browser windows converge when you switch guilds (storage events), and an `X-Resolved-Guild` response echo lets the app discard in-flight responses from a context you've already left.
...
- **Backend notifications are now localized.** Every backend-generated notification — calendar events (invitation, reschedule, update, cancellation, RSVP, reminder), @-mentions, comments, comment replies, task assignments, initiative/project additions, and access-grant (PAM) requests/approvals/denials/revocations — now renders its email **and** push copy in the recipient's language (English, Spanish, French) instead of always English. The i18n loader gained a channel-split structure: rich email copy (subject/title/body) lives in the `email` namespace and terse push copy (title/body) in a new `notifications` namespace, plus an English fallback so an untranslated locale shows English rather than a raw key.
- **The mobile app sidebar now follows your finger.** You can swipe in from the left screen edge to pull the sidebar open, and swiping it closed animates from wherever you release instead of snapping fully open first. The open and close animations are now driven by a single transform, fixing the glitch where a swipe-to-close briefly jumped the drawer back open before sliding it shut.
...
- **Uploads and document downloads no longer 404 under schema-per-guild.** The `/uploads` and document/version download routes looked up their records only in the `public` schema, but new content is recorded in the guild's own schema — so freshly uploaded images and documents failed to serve (intermittently masked by connection reuse). Both now resolve the record across the requester's own guild schemas, fail-closed; non-members always get 404 so existence is never confirmed.
- **Account deletion, anonymization, and deactivation now actually clean up guild content.** The deletion cascades, eligibility/blocker checks, and project-transfer flow ran on an unrouted admin session, so under schema-per-guild they read/wrote the dead `public` copies and left the departing user's memberships, permissions, and authored content behind in every guild (and under-reported deletion blockers). They now route into each of the user's guild schemas. The platform-admin "delete guild" now also drops the guild's schema, and "delete initiative" / the initiative member-pickers and role changes now take a `guild_id` and route correctly.
- **OIDC login role sync works under schema-per-guild.** `sync_oidc_assignments` read initiatives/roles/memberships from the dead `public` copies, silently dropping SSO-driven initiative assignments; it now resolves and applies each guild's memberships routed into that guild's schema.
- **Cross-guild calendar views read live data.** The global calendar list and `.ics` export aggregated from `public`; they now gather per guild like the global task/document lists.

### Security

- **Email-bound guild invites now enforce the bound address (SEC-15).** Redeeming an invite checked only expiry and remaining uses, never that the redeeming user's email matched the invite's `invitee_email` — so an email-targeted invite link could be redeemed by anyone who obtained the code, giving a false sense of email binding. Redemption now rejects a mismatched user with `INVITE_EMAIL_MISMATCH` (compared via the same normalization as the email unique constraint); invites with no bound email remain shareable links.
- **Hardened HTTP/docs/SMTP exposure (SEC-16).** Responses now carry `Strict-Transport-Security` (2 years, `includeSubDomains`) when `APP_URL` is HTTPS, so browsers refuse plain-HTTP downgrades. The interactive API docs and raw OpenAPI schema are now gated behind a new `ENABLE_API_DOCS` setting (default `true` for local dev; set `false` in production so the full route/parameter/error map isn't publicly served). The SMTP "send test email" endpoint no longer returns the raw SMTP exception (which could leak the mail host, port, or server banner) — it now returns a generic `SETTINGS_EMAIL_SEND_FAILED` code and logs the real cause server-side only.
- **Rate limiting now applies to every route, and "all rows" list requests are capped (SEC-14).** The global default limit was inert because `SlowAPIMiddleware` was never registered — only ~14 decorated routes were throttled; the middleware is now wired up so the per-client default covers undecorated routes too. The default limit and counter storage are settings-driven (`RATE_LIMIT_DEFAULT`, `RATE_LIMIT_STORAGE_URI`) so a multi-worker deploy can point at Redis without a code change. List endpoints that accept `page_size=0` ("all rows") are now bounded by a hard server-side ceiling (`MAX_UNBOUNDED_PAGE_SIZE`, default 1000) instead of dumping an entire table.
- **Device tokens are now hashed at rest and expire (SEC-13).** Device, email-verification, and password-reset tokens are stored as SHA-256 hashes instead of plaintext (matching API keys), and the lookup hashes the presented value before comparing. Device tokens drop from an effectively unexpiring ~100-year lifetime to a sliding 90-day window that refreshes on use, so abandoned tokens die within the cap while active devices stay signed in. A migration hashes existing rows in place (so current devices and in-flight links keep working) and caps the legacy long-lived expiries to 90 days.
- **Plain-text fields are no longer HTML-encoded when saved.** The schema-level sanitizer ran an HTML *output encoder* (`nh3.clean`) on every string field, storing benign characters as their HTML entities — a name typed as "Foo & Bar" was saved (and shown) as "Foo &amp; Bar". Plain-text fields are now stripped of markup *without* entity-encoding, so user input round-trips exactly while `<img onerror>`/`<script>` payloads are still removed — including entity-encoded forms (`&lt;img onerror&gt;`) at any depth, which a naive unescape-after-strip would have revived. A one-time data migration to repair already-encoded rows is still pending.
- **CORS no longer reflects arbitrary origins on credentialed requests (CRIT-001).** The allowlist defaulted to `*`, which combined with `allow_credentials=True` let any website make authenticated cross-origin requests on a logged-in user's behalf. The effective allowlist is now explicit — always including `APP_URL` and the native app origins (each normalized to its bare `scheme://host:port` so an `APP_URL` with a path still matches), plus any `CORS_ALLOWED_ORIGINS` — and a wildcard is dropped. Bundled (same-origin) and local-dev deployments need no config; the effective allowlist is logged at startup.
- **A Content-Security-Policy is now sent on every response (MED-001).** Scripts are confined to same-origin (`script-src 'self'`, no `unsafe-inline`/`unsafe-eval`), with `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, and `frame-ancestors 'none'`. Origins the app actually uses (Google Fonts, document embeds, and — when configured — the captcha provider and advanced-tool iframe) are allowlisted explicitly. The Google-Fonts `<link>` no longer uses an inline `onload` handler so the policy doesn't break font loading.
...
- **Document links can no longer carry dangerous URLs.** Link URLs in collaborative documents are now run through a strict protocol allowlist (http/https/mailto/tel/sms and relative links) on import, render, and before opening — so a stored `javascript:`/`data:`/`vbscript:` link renders inert and cannot execute.
- **Transactional emails no longer render user-supplied markup.** Interpolated values (display names, resource titles, mention text) are HTML-escaped in HTML email bodies, so a display name like `<a href="…">Reset your password</a>` shows as literal text instead of a live phishing link inside the trusted, brand-styled email. Subjects and plain-text alternatives keep the raw text.
- **`SECRET_KEY` is now validated at startup.** The app refuses to boot when the key is unset, a known placeholder (`change-me`, `super-secret-key`), or shorter than 32 characters — this one key signs session JWTs and the OIDC state HMAC and roots all field encryption. The compose example now requires an explicit `SECRET_KEY` (`openssl rand -hex 32`) and the example token lifetime was lowered to 60 minutes.
...
- OIDC logins now require the identity provider to assert `email_verified` before linking to an existing local account, closing a pre-registration account-takeover path; unverified claims no longer silently mark accounts verified.
